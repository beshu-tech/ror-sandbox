...
import_saved_obj () {
    local USR=$1
    local PASS=$2
    DEST_ID=$(curl -s -u "$USR":"$PASS" -XPOST -H "kbn-xsrf: true" "http://kbn-ror:5601/api/saved_objects/_import?createNewCopies=true" --form file=@export-ror.ndjson | jq -r .successResults[0].destinationId)
    echo "$DEST_ID"
}
