...
import_saved_obj () {
    local USR=$1
    local PASS=$2
    DEST_ID=$(curl -sk -u "$USR":"$PASS" -XPOST -H "kbn-xsrf: true" "https://kbn-ror-01:5601/api/saved_objects/_import?createNewCopies=false" --form file=@export-ror-$USR.ndjson | jq -r .successResults[0].id)
    echo "$DEST_ID"
}
